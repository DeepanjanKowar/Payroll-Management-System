...
if (isset($_POST['insert'])) {
  ...
  <?php
mysql_connect("mysql11.000webhost.com", "a4018284_jade" , "1234jadelanga" )or die("cannot connect to database server");  
	mysql_select_db("a4018284_payroll")or die("cannot select the database");
...
while ($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
  ...
  echo '<tr><td>Middle I<td>:</td><td></td></td><td>'.$row['init'].'</td></tr>';
   echo '<tr><td>Department<td>:</td><td></td></td><td>'.$row['dept'].'</td></tr>';
  echo '<tr><td>Position<td>:</td><td></td></td><td>'.$row['position'].'</td></tr>';
...
</style>  <fieldset class="a" > <legend>Add Entry</legend>
...
<input type="text"name="id" readonly class="id"value=<?php echo $id; ?> tabindex="12" title="";} />   
    <?php 
	$result="";
if (isset($_POST["field"])) {
  $key = strtoupper($_POST["key"]);
